defmodule RumblWeb.AuthTest do
  ...
  test "authenticate_user halted when user session is empty", %{conn: conn} do
    conn = Auth.authenticate_user(conn, [])

    assert conn.halted
  end

  test "authenticate_user when user session is full", %{conn: conn} do
    conn =
      conn
  ...
  test "call places user from sessoin into assigns", %{conn: conn} do
    user = user_fixture()
